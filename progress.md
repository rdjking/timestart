...
## 2026-07-18：节假日规则下次执行日期修正

- 用户真机发现：法定工作日任务仍把周日显示/登记为下一次执行。根因是旧实现每日登记闹钟、到触发时才判断节假日类型。
- 正在改为查询并跳过不匹配日期后，直接登记下一个实际符合规则的时间；该逻辑将覆盖保存、启用、恢复和触发后重调度。
- 已完成：`NextTriggerCalculator` 直接查找下一个匹配的节假日日期；`TaskRepository`、`TaskRescheduler`、`TaskTriggerCoordinator` 与应用启动恢复均已接入同一日历。
- 回归测试通过：计算器的“跳过周日”场景、Repository 保存场景，以及既有任务触发测试均为 0 失败、0 错误。
- 真机验证：通过 USB 设备 `49GUFQHUZ5LBRSCI` 安装并打开应用后，首页摘要和 QQ 任务卡片均显示 `07月20日 20:09`，不再显示 `07月19日`。
- 记录：全量 Gradle 测试任务在当前终端环境中会在实际测试前提前结束且不产出完整报告；未将该次运行报告为通过，改以受影响路径的定向测试和真机结果验收。
