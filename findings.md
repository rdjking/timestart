...
## 2026-07-18：节假日规则的下次触发策略

- 仅在闹钟触发时判断日期类型会让首页把不匹配日期误显示为“下一次执行”，也会无意义地为周末登记精确闹钟。
- 正确策略是：从候选日期开始逐日查询 `HolidayCalendar`，直接持久化并登记第一个匹配当前规则的日期；触发时保留二次校验作为数据更新或旧闹钟残留时的防御。
- 因此任务保存、重新启用、开机/时区恢复、任务触发后的再次调度必须使用同一个带节假日日历的 `NextTriggerCalculator`。
